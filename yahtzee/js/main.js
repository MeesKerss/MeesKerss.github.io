...
function sndImpact(intensity) {
  ...
  oscTable.frequency.setValueAtTime(150, now); oscTable.frequency.exponentialRampToValueAtTime(60, now + 0.04);
...
function sndClick(isLocking) {
  ...
  g.gain.setValueAtTime(0.07,a.currentTime); g.gain.exponentialRampToValueAtTime(0.001,a.currentTime+0.055);
...
function sndScore() {
  ...
  [[440,0],[660,0.09]].forEach(([f,t]) => {
    ...
    g.gain.setValueAtTime(0,a.currentTime+t); g.gain.linearRampToValueAtTime(0.22,a.currentTime+t+0.015);
    g.gain.exponentialRampToValueAtTime(0.001,a.currentTime+t+0.22);
    osc.connect(g); g.connect(a.destination); osc.start(a.currentTime+t); osc.stop(a.currentTime+t+0.22);
  });
}
function sndYahtzee() {
  const a=ac();
  [523,659,784,1047].forEach((f,i) => {
    ...
    g.gain.setValueAtTime(0,a.currentTime+i*0.11); g.gain.linearRampToValueAtTime(0.28,a.currentTime+i*0.11+0.02);
    g.gain.exponentialRampToValueAtTime(0.001,a.currentTime+i*0.11+0.32);
    osc.connect(g); g.connect(a.destination); osc.start(a.currentTime+i*0.11); osc.stop(a.currentTime+i*0.11+0.32);
  });
}
...
function getMaterialsForValue(v, faceUp, srcMats) {
  ...
  const avail = [[1,6],[2,5],[3,4]].filter(p => p[0]!==v && p[1]!==v && p[0]!==7-v);
  let aIdx = 0;
  for(let i=0; i<3; i++) {
...
function init3D() {
  ...
  window.addEventListener('click', (e) => {
    if(!G || !G.rolled || G.rolling || G.rollsLeft===0) return;
    ...
    if(rollingDice.length > 0) {
      ...
      for(let rd of rollingDice) {
        if(rd.frame < rd.traj.length) {
          ...
          const b = rd.bounces.find(x => x.frame === rd.frame);
          if(b) sndImpact(b.intensity);
          rd.frame++;
        }
      }
...
function alignHeldTags() {
  ...
  const rect = appEl.getBoundingClientRect();
  for(let i=0; i<5; i++) {
    const tag = document.getElementById(`htag-${i}`);
    const tgt = getTargetPos(i);
    const pos = new THREE.Vector3(tgt.x, 0, tgt.z + 1.2);
    pos.project(camera);
    tag.style.left = (pos.x*0.5+0.5)*rect.width + 'px';
    tag.style.top = (pos.y*-0.5+0.5)*rect.height + 'px';
  }
}
function updateHeldTags() {
  for(let i=0; i<5; i++) {
    const tag = document.getElementById(`htag-${i}`);
    if(G.locked[i] && !G.rolling) tag.classList.add('on');
    else tag.classList.remove('on');
  }
}

function precalculateRoll(tx, tz, targetValue) {
  ...
  die.position.set((Math.random()-0.5)*5, 12+Math.random()*4, startZ);
  ...
  for(let i=0; i<85; i++) {
    ...
    world.step(dt);
    if(lastDy < 0 && die.velocity.y > 0 && Math.abs(lastDy) > 0.5) {
      bounces.push({ frame: i, intensity: Math.abs(lastDy) });
    }
  ...
  for(let i=0; i<6; i++) {
    ...
    if(dot > max) { max = dot; faceUp = i; }
  }

  return { traj, bounces, faceUp };
}
...
function pickScore(id) {
  ...
  setTimeout(() => {
    ...
    updateHeldTags(); updateHeader(); updateRollBtn(); buildScorecard(); flashNotif();
  }, 400);
}

// UI Rendering
function buildScorecard() {
  const t = document.getElementById('sc'); t.innerHTML = '';
  const hr=t.createTHead().insertRow(); th(hr,'');
  G.players.forEach((p,i)=>{
    const el=th(hr, p.name.length>9?p.name.slice(0,9)+'…':p.name);
    if(i===G.cur){el.classList.add('cur');el.style.color=p.color;}
  });
  const tbody=t.createTBody();
  sect(tbody,'Upper Section'); CATS.filter(c=>c.sec==='U').forEach(c=>catRow(tbody,c)); bonusRow(tbody);
  sect(tbody,'Lower Section'); CATS.filter(c=>c.sec==='L').forEach(c=>catRow(tbody,c)); totRow(tbody);
}
function th(row,txt){const el=document.createElement('th');el.textContent=txt;row.appendChild(el);return el;}
function sect(tbody,lbl){const tr=tbody.insertRow();tr.className='sect';const td=document.createElement('td');td.colSpan=G.players.length+1;td.textContent=lbl;tr.appendChild(td);}
function catRow(tbody,cat){
  ...
  G.players.forEach((p,i)=>{
    const td=document.createElement('td');td.className='scell'+(i===G.cur?' cur':'');
    const sh=G.sheets[i],sv=document.createElement('span');sv.className='v';
    if(sh[cat.id]!==undefined){sv.classList.add('done');sv.textContent=sh[cat.id];}
    else if(i===G.cur&&G.rolled){
      const s=calcScore(cat.id,G.dice);
      sv.classList.add(s>0?'pot':'zero');sv.textContent=s>0?s:'0';
      td.addEventListener('click',()=>pickScore(cat.id));
    } else {sv.classList.add('blank');sv.textContent='—';}
    td.appendChild(sv);tr.appendChild(td);
  });
}
function bonusRow(tbody){
  const tr=tbody.insertRow();tr.className='bon';
  const tdn=document.createElement('td');tdn.className='cname';tdn.textContent='Bonus (≥63 → +35)';tdn.style.color='#bbb';tr.appendChild(tdn);
  G.players.forEach((p,i)=>{
    const td=document.createElement('td');td.className='scell'+(i===G.cur?' cur':'');
    const sh=G.sheets[i],us=upperTotal(sh),b=upperBonus(sh),filled=UPPER.filter(k=>sh[k]!==undefined).length;
    const sv=document.createElement('span');sv.className='v';
    if(b>0){sv.classList.add('bon-on');sv.textContent='+35';}
    else if(filled>0){sv.classList.add('bon-pr');sv.textContent=`${us}/63`;}
    else{sv.classList.add('blank');sv.textContent='—';}
    td.appendChild(sv);tr.appendChild(td);
  });
}
function totRow(tbody){
  const tr=tbody.insertRow();tr.className='tot';
  const tdn=document.createElement('td');tdn.textContent='Total';tr.appendChild(tdn);
  G.players.forEach((p,i)=>{
    const td=document.createElement('td');td.className='scell'+(i===G.cur?' cur':'');
    const sv=document.createElement('span');sv.className='v tot';sv.textContent=grandTotal(G.sheets[i]);
    if(i===G.cur)sv.style.color=p.color;
    td.appendChild(sv);tr.appendChild(td);
  });
}
...
document.getElementById('start-btn').addEventListener('click',()=>{
  ...
  init3D(); updateHeader(); updateRollBtn(); buildScorecard(); flashNotif();
});
